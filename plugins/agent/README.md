...
| 状态 | 小人 | 徽标 |
| --- | --- | --- |
| idle（没在跑） | 闭眼（眼缝变一横） | 头部右上角 3×3 灰 `Z` 斜着往右上飘 |
| busy（正在生成 / 跑工具） | 扛锄头走路：四条腿交替长短 + 锄头两帧挥动（锄头钢灰蓝 `#C9D4E0`） | 无 |
| wait（等你批准权限） | **瞪大眼**（眼缝加倍宽） | **5×9 黄色大问号常亮 + 整屏 1px 橙边框闪烁** |

wait 是唯一需要你动手的状态，所以给了三重信号：大问号、闪烁边框、瞪大眼——
余光扫到闪框就知道要回去看。边框画在最底层，小人盖在它上面。
...
`Notification` 仍然要注册，但它**不映射状态**：回合结束时它在 `Stop` 之后 fire，
映射成 wait 的话会把刚写好的 idle 盖掉，让做完的会话一直显示大问号。
留着它只是为了刷新 `ts`，把会话保持在 180 秒活跃窗口内。
